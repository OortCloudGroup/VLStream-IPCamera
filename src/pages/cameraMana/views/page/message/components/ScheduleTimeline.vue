...
const legend: LegendItem[] = [
  { key: 'off', label: '关闭', color: '#999999' },
  { key: 'patrol', label: '巡航扫描', color: '#1AC5FF' },
  { key: 'preset', label: '预置点', color: '#15CF87' },
  { key: 'reboot', label: '球机重启', color: '#FF821A' },
  { key: 'check', label: '球机校验', color: '#841AFF' },
  { key: 'aux', label: '辅助输出', color: '#FF1A98' }
]

const selectedLegendKey = ref<string>(legend[0]?.key || '')

const getLegendColor = (key: string): string => {
  const item = legend.find(l => l.key === key)
  return item ? item.color : '#1AC5FF'
}
  ...
  if (repeat === 'day') {
    ...
    const segments: Seg[] = periods
      ...
      .map((p: any, index: number) => ({
        ...
        type: (p as any).type || 'check',
        color: getLegendColor((p as any).type || 'check')
      }))
