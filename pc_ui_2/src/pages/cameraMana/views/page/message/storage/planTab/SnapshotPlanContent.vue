<template>
  <div>
    ...
    <div class="repeat_row">
      <div class="repeat_tabs">
        <div
          ...
          :key="item.value"
          class="repeat_tab"
          :class="{ act: formValue.repeat === item.value }"
    ...
    <el-form v-if="formValue.repeat === 'next'" class="interval_form" label-position="top" :model="formValue">
      <el-form-item label="每隔">
        <el-input :model-value="formValue.interval" class="interval_input" @update:model-value="updateInterval">
          <template #suffix>
            天
          </template>
        </el-input>
    ...
    <div class="timeline_wrap">
      ...
      <div v-for="day in dayRows" :key="day.key" class="timeline_row">
        ...
        >
          ...
          <div v-if="formValue.repeat === 'week' || formValue.repeat === 'month'" class="row_copy" @click.stop="handleCopyClick(day.key)">
    ...
    >
      <plan-tab-new :item="advancedParams" @close="messVisi = false" @handle="applistFn" />
    </el-dialog>
  </div>
</template>
...
const emit = defineEmits<{
  ...
  'advanced-params-handle': [payload: { loop_writing: boolean; pre_record_time: number; record_delay: number; code_stream_type: string }]
}>()
...
const formValue = computed(() => props.form || defaultForm)

const messVisi = ref<boolean>(false) // 高级参数 dialog
...
const convertSegmentsToTimePeriods = (segments: Record<string, Seg[]>, repeat: string, interval: number): TimePeriods => {
  if (repeat === 'day') {
...
watch(
  () => [props.timePeriods, formValue.value.repeat],
  (newVal, oldVal) => {
    if (props.timePeriods !== undefined) {
      ...
      if (newRepeat !== oldRepeat) {
        ...
        const converted = convertTimePeriodsToSegments(props.timePeriods, formValue.value.repeat)
        getRepeatKeys(formValue.value.repeat).forEach(key => {
...
watch(
  () => [segmentsByDay, formValue.value.repeat, formValue.value.interval],
  () => {
...
const openEditPop = (dayKey: string, segId: string, anchorEl?: HTMLElement | null) => {
  ...
  // 默认居中显示，若有锚点则显示在时间段上方
  let left = (window.innerWidth - 420) / 2
...
const handleSegmentDelete = (dayKey: string, segId: string) => {
  ...
  if (editPop.visible && editPop.segId === segId) {
    closeEditPop()
  }
}
...
const dayRows = computed(() => {
  if (formValue.value.repeat === 'month') {
    // 每月模式：显示1-31天
    return Array.from({ length: 31 }, (_, i) => ({
      key: `day${i + 1}`,
      label: `${i + 1}`
    }))
  } else if (formValue.value.repeat === 'week') {
    // 每周模式：显示星期一到星期日
    return [
      { key: 'mon', label: '星期一' },
      { key: 'tue', label: '星期二' },
  ...
  } else if (formValue.value.repeat === 'next') {
...
const segmentStyle = (seg: Seg) => {
  ...
  // 如果时间段为0或太小，至少显示一个最小宽度用于视觉反馈（仅用于显示，不影响实际数据）
  if (duration <= 0 || duration < 0.1) {
    const minWidthPercent = (0.1 / 24) * 100 // 最小0.1小时对应的百分比
...
const copyDaysOptions = computed(() => {
  if (formValue.value.repeat === 'week') {
    return copyWeekDays
  } else if (formValue.value.repeat === 'month') {
    return Array.from({ length: 31 }, (_, i) => ({
      label: `${i + 1}`,
      value: `day${i + 1}`
    }))
...
const handleCopyConfirm = () => {
  ...
  copyDays.value.forEach(targetDayKey => {
    // 跳过源天本身
    if (targetDayKey === sourceDayKey.value) {
      return
    }
...
const getTimeFromMouseX = (e: MouseEvent, trackElement: HTMLElement): number => {
  ...
  return Math.round(time * 100) / 100 // 保留两位小数
}
...
const handleTrackMouseDown = (e: MouseEvent, dayKey: string) => {
  ...
  // 如果点击在复制按钮上，不处理
  if (target.closest('.row_copy')) return
  // 如果点击在删除按钮上，不处理
  if (target.closest('.seg_delete')) return
  // 如果点击在调整大小手柄上，不处理
  if (target.closest('.seg_resize_left') || target.closest('.seg_resize_right')) return
...
const handleSegmentMouseDown = (e: MouseEvent, dayKey: string, seg: Seg) => {
  ...
  if ((e.target as HTMLElement).classList.contains('seg_resize_left') ||
      (e.target as HTMLElement).classList.contains('seg_resize_right') ||
