<template>
  <div class="meeting_record">
    ...
    >
      ...
      <el-table-column prop="name" label="结论">
        <template #default="scope">
        ...
        >
          <div class="flexRowAC voiceUp">
            <el-icon class="el-icon--upload">
              <img src="@/assets/img/audioVideo_.png" alt="" />
            </el-icon>
            <div class="voiceUp_t flexRowAC">
              上传结论
            </div>
            <!--            <div class="voiceUp_t_tip">-->
            <!--              文件仅支持Word、Excel、PPT、PDF文件格式-->
            <!--            </div>-->
          </div>
        </el-upload>
        <div class="filesUp">
...
let isTitle = ref<any>(false)
let titleInput = ref<any>('')
...
const materialAddFn = async() => {
  if (!fileList.value.length) return ElMessage.warning('请上传结论')
  let data = {
    accessToken: store.userInfo.accessToken,
...
// 删除
const delClick = (rows) => {
  ElMessageBox.confirm('是否删除该结论？', '提示', {
