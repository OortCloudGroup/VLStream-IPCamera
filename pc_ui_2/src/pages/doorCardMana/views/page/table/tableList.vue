  ...
  <div class="m_table">
    <div class="table-card-list-container">
      ...
      >
        ...
        <el-table-column prop="name" label="门牌编号" />
...
import { getDoorCardList, tableImgList } from '@/api/tableService/index_door'
...
const prviewCard = (data) => {
  ...
  if (!currentTableItem.value.image) {
    ElMessage.warning('该门牌未设置过模板或者模板已删除')
    return
  }
...
const getList = () => {
  ...
  getDoorCardList(params).then((res) => {
    if (res.code === 200) {
...
import { saveDoorCardInfo } from '@/api/tableService/index_door'
...
const confrimAddDevice = () => {
  ...
  saveDoorCardInfo(params).then((res) => {
    if (res.code === 200) {
      ElMessage({
        type: 'success',
        message: '操作成功'
