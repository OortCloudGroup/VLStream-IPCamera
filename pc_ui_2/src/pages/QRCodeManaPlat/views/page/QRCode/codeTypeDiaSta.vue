...
import { scanStatisticsTypeCount, scanStatisticsTypeCountByDate } from '@/api/QRcode'
...
const getTodayScanData_Fn = async() => {
  let data = props.itemObj.qrTypeId
  let res: any = await scanStatisticsTypeCountByDate(data)
  if (res.code === 200) {
...
const scanStatisticsCount_Fn = async() => {
  ...
  let res: any = await scanStatisticsTypeCount(data, data1)
  if (res.code === 200) {
