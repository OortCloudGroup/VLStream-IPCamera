...
import { deptList, getPolicePersonalStatisticsByDepartment } from '@/api/mobileeducationplatform/case'
...
const getList = async() => {
  ...
  const res = await getPolicePersonalStatisticsByDepartment(params)
...
const handleExport = async() => {
  ...
  const downloadUrl = Config.URL + Config.gateWay + 'oortcloud-educationtalk' + '/report/exportPolicePersonalStatisticsByDepartment'
  let res = await request({ url: downloadUrl, method: 'post', data: params, responseType: 'blob' }, true)
  ElMessage.success('正在导出中...')
  downloadBlob(res, '民警个人谈话统计.xlsx')
}
