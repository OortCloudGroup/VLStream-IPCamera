      ...
      <div class="content-area">
        <div class="filter-bar">
          ...
          <div class="filter-item">
            ...
            <el-select v-model="filters.talkCategory" placeholder="谈话类型" style="width: 210px">
              <el-option label="全部" value="" />
              <el-option
...
import { deptList, getCaseTypes, getTalkTypeStatistics } from '@/api/mobileeducationplatform/case'
...
const getList = async() => {
  ...
  const res = await getTalkTypeStatistics(params)
...
const handleExport = async() => {
  ...
  const downloadUrl = Config.URL + Config.gateWay + 'oortcloud-educationtalk' + '/report/exportTalkTypeStatistics'
  let res = await request({ url: downloadUrl, method: 'post', data: params, responseType: 'blob' }, true)
  ElMessage.success('正在导出中...')
  downloadBlob(res, '谈话类型统计.xlsx')
}
