...
import { deptList, getPrisonerDurationStatisticsByDepartment } from '@/api/mobileeducationplatform/case'
...
const getList = async() => {
  ...
  const res = await getPrisonerDurationStatisticsByDepartment(params)
...
const handleExport = async() => {
  ...
  const downloadUrl = Config.URL + Config.gateWay + 'oortcloud-educationtalk' + '/report/exportPrisonerDurationStatisticsByDepartment'
  let res = await request({ url: downloadUrl, method: 'post', data: params, responseType: 'blob' }, true)
  ElMessage.success('正在导出中...')
  downloadBlob(res, '罪犯谈话时长.xlsx')
}
