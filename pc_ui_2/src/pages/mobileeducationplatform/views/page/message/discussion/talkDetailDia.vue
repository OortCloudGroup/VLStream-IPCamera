<template>
  <div class="dialog-content">
        ...
        >
          ...
          <div class="file-list">
            ...
            >
              ...
              <span class="file-name">{{ getWordFileName(talkDetailData.recordNo, talkDetailData.audioPath) }}</span>
            </div>
            <div
            ...
            >
              ...
              <span class="file-name">{{ getWordFileName(talkDetailData.recordNo, talkDetailData.videoPath) }}</span>
            </div>
            <div
    ...
    >
      <!-- 音频预览 -->
      <audio v-if="filePreviewVisible && previewType === 'audio'" :src="previewUrl" controls style="width: 100%;max-height: 65vh;" />

      <!-- 视频预览 -->
      <video v-if="filePreviewVisible &&previewType === 'video'" :src="previewUrl" controls style="width: 100%;max-height: 65vh;" />

      <!-- 文本预览 -->
      <div v-if="filePreviewVisible && previewType === 'word'" v-loading="textLoading" class="word-preview">
...
const talkFilesManaFn = () => {
  ...
  // let url = Config.URL.slice(0, -1) + Config.frontURLStr + 'cloud_disk/index.html?&folder=教育谈话&accessToken=' + store.userInfo.accessToken
  // window.open(url, '_blank')
}

const handleFilePreview = (type: any, path: any) => {
...
const handleFileDownload = async(path: any, recordNo: any) => {
  try {
...
const getWordFileName = (recordNo: any, path) => {
  // 返回路径的 name
  const queryString = getQueryUrlString(path)
  console.log('queryString', queryString)
  const name = queryString.name
  if (name) {
    return name
  } else {
    return `${recordNo}.docx`
  }
}
...
// }
const talkDetailData = ref(null)

async function fetchTalkDetail(id: any) {
  try {
    const response = await getMemconInfo(id)
    if (response.code === 200) {
      talkDetailData.value = response.data
