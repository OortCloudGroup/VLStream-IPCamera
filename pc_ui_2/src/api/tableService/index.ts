...
function commonFunc<T, K>(interfaceName: string, data: T, method: string) {
  return request<K>({
    url: config.URL + config.gateWay + 'oortcloud-table/' + interfaceName,
    method: method,
    data: data
  })
}
...
export function tableImgDelete(data) {
  return commonFunc('table/v2/img_delete', data, 'post')
}

export function tableImgEdit(data) {
  return commonFunc('table/v2/img_edit', data, 'post')
}

export function tableImgInfo(data) {
  return commonFunc('table/v2/img_info', data, 'post')
}

export function tableImgList(data) {
  return commonFunc('table/v2/img_list', data, 'post')
}

export function tableImgSetDefault(data) {
  return commonFunc('table/v2/img_set_default', data, 'post')
}

export function tableImgUpload(data) {
  return commonFunc('table/v2/img_upload', data, 'post')
}
