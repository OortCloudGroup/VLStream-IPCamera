<template>
  <div class="es-layout-container">
    ...
    <Info v-model="store.current" :size="size" />
  </div>
</template>
...
const editorContainerStyle = computed(() => {
  ...
  const x = store.initWidth > width
  const y = store.initHeight > height

  console.log('x', x, 'y', y, store.initWidth, store.initHeight, width, height)
...
function drop(e: DragEvent) {
  ...
  const isIcon = currentComponent.component === 'EsIcon'
  const elements = [
    ...store.data.elements,
    {
      ...currentComponent,
...
function init() {
  store.initWidth = Math.floor(mainRect.value.width) || 1180
  store.initHeight = Math.floor(mainRect.value.height) || 960

  //  store.initWidth = Math.floor(mainRect.value.width) || props.size[0]
  // store.initHeight = Math.floor(mainRect.value.height) ||  props.size[1]
