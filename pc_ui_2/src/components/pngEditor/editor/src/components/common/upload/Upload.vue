...
import { uploadBase64 } from '@/api/fastdfs'
const handleChange = async(e: Event) => {
  ...
  if (['json', 'text'].includes(resultType)) {
    ...
    if (file.size > props.option.limitSize * 1024 * 1024) {
      alert(`图片大小不能超过${props.option.limitSize}M`)
      return
    }
    // 上传到fastdfs
    const params = {
    ...
    let res = await uploadBase64(params)
    if (res.code === 200) {
      result = res.data.url
    }
  }
 ...
 * 读取文件的文本内容
 */
const readFile = (file: File, type = 'text') => {
